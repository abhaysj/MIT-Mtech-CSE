...
// Define a global variable to keep track of the potential (amortized cost)
int potential = 0;

// Define a global variable to keep track of the accumulated cost
int accumulated_cost = 0;

// Define a function to update the potential
void updatePotential(int delta) {
    potential += delta;
}

// Define a function to update the accumulated cost
void updateAccumulatedCost(int cost) {
...
void insertion(FIB_HEAP *H, NODE *new, int val) {
  ...
  (H->n)++;
   int actual_cost = 1;  // Or whatever the actual cost is

    // Method 1: Aggregate
    // Calculate the change in potential (using the number of nodes as potential)
    int delta_aggregate = 1;  // For this simple example, we assume each insertion adds 1 to potential

    // Update the potential using the aggregate method
    updatePotential(delta_aggregate);

    // Calculate the amortized cost using the aggregate method
    int amortized_aggregate = actual_cost + delta_aggregate;

    // Method 2: Accounting
    // Overcharge for the operation to pay for future expensive operations
    int overcharge_accounting = 2;  // For this simple example, we overcharge by 2

    // Update the accumulated cost using the accounting method
    updateAccumulatedCost(actual_cost + overcharge_accounting);

    // Calculate the amortized cost using the accounting method
    int amortized_accounting = accumulated_cost;

    // Method 3: Potential
    // Calculate the change in potential (using the number of nodes as potential)
    int delta_potential = 1;  // For this simple example, we assume each insertion adds 1 to potential

    // Update the potential using the potential method
    updatePotential(delta_potential);

    // Calculate the amortized cost using the potential method
    int amortized_potential = actual_cost + delta_potential;
printf("Amortized Cost (Aggregate): %d\n", amortized_aggregate);
    printf("Amortized Cost (Accounting): %d\n", amortized_accounting);
    printf("Amortized Cost (Potential): %d\n", amortized_potential);
}
